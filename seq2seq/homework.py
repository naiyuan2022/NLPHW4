...
def read_data():
    with open('倚天屠龙记.txt', "r", encoding='GB18030') as f:
...
def train():
    ...
    for epoch_id in range(epochs):
        for idx in trange(0, len(sequences) // end_num - 1):
            ...
            optimizer.zero_grad()
            out_res = model(all_seq[:-1].cpu())
            f1 = ((out_res[-target_seq.shape[0]:] ** 2).sum(dim=1)) ** 0.5
            f2 = ((target_seq.cpu() ** 2).sum(dim=1)) ** 0.5
            loss = (1 - (out_res[-target_seq.shape[0]:] * target_seq.cpu()).sum(dim=1) / f1 / f2).mean()
            loss.backward()
            optimizer.step()
            if idx % 50 == 0:
...
def test():
    ...
    input_seq = torch.zeros(len(seqs), embed_size).cpu()
    result = ""
    with torch.no_grad():
        ...
        while end_num < 10 and length < 2000:
            print("length: ", length)
            out_res = model(input_seq.cpu())[-2:-1]
